...
kernel void actInp(constant int *outArr [[buffer(0)]],
                   ...
                   device atomic_uint *contrArr [[buffer(2)]],
    ...
    int xDim = numConnx*blockDim.x * gridDim.x;
    for (int i = xPos; i < numOut; i+=xDim) {
        for (int j = 0; j < numConnx; j++) {
            int inpInd = outArr[numConnx*i + j];
            if (inpInd == actVect[yPos]) {
                uint incr = uint(pow( 2, float(4*(i%8))));
                atomic_fetch_add_explicit(contrArr + int(i / 8) , incr, memory_order_relaxed);
